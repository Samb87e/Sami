...
function makePayment() {
    ...
    // New logic for automated payments
    const automatedTransaction = async () => {
        ...
        let sequenceNumber = parseInt(accountA.sequence);
        ...
        const sendPayment = async () => {
            ...
            try {
                ...
                console.log(chalk.magentaBright(`Transaction submitted successfully! Hash: ${response.hash}`));
            } catch (error) {
                console.error(chalk.red('Transaction failed:', error.response ? error.response.data.extras.result_codes.operations : error.message));
            }
        };

        // Loop to send transactions every 150 milliseconds
        setInterval(sendPayment, 150);
    };

    automatedTransaction().catch(fail);
}
